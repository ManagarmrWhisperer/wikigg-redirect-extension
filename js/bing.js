...
const filter = {
    ...
    run( wiki, linkElement ) {
	    ...
	    const resultContainer = document.querySelector( this.ENGINE_RESULT_LIST_CONTAINER );
            const oldElement = linkElement.closest('.slide:has(.b_algo)') || linkElement.closest( ' .b_algo, .b_algo_group, #wikiWidgetContainer', '.pagereco_CBTextCard');
            // If we're hidden - skip, we were already here
            if ( oldElement.style.display === 'none' ) {
                return;
            }
	    // We can't move wiki cards, and only one widget container exists
...
const rewrite = {
    ...
    rewriteLink( wiki, link ) {
        if ( link.href.startsWith( '/url?' ) ) {
            ...
            link.href = link.href.replace(new RegExp( `(https:\/\/)?${wiki.oldId || wiki.id}.fandom.com`), `https://${wiki.id}.wiki.gg` );
        }
    },
    ...
    run( wiki, linkElement ) {
        if ( linkElement !== null  ) {
            // Find result container
	    // NOTE: This should get optimized.
            const element = linkElement.closest( ' .slide:has(.b_algo) ' ) || linkElement.closest( ' .b_algo, .b_algo_group, #wikiWidgetContainer', '.pagereco_CBTextCard' );
		...
		for ( const title of element.querySelectorAll( this.NETWORK_TITLE_SELECTOR )) {
		    if ( title.textContent === 'Fandom' )
			title.textContent = "Wiki.gg"
		}
		...
		element.querySelectorAll( '.tltg' ).forEach(tag => tag.textContent = tag.textContent.replace('#Fandom', '#Wiki.gg'))
		this.lock( element );
            }
